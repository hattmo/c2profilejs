...
export default ({ path, label, formatKey, formatValue, selectedOptions = [], onChanged }: IProps) => {
    ...
    return (
        <div>
            ...
            <input type="text" className={validateKey()} onChange={(e) => { setKey(e.currentTarget.value) }} value={key} />
            <input type="text" className={validateValue()} onChange={(e) => { setValue(e.currentTarget.value) }} value={value} />
